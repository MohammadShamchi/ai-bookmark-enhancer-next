 ...
 * @param {Function} options.onProgress - Optional callback for progress updates (receives object)
...
async function simulateFastMode(flatList, { onProgress, signal }) {
  ...
  const chunks = chunkArray(flatList, Math.max(50, Math.floor(CHUNK_SIZE / 2)));
  const totalChunks = Math.max(chunks.length, 1);

  if (onProgress) {
    onProgress({
      percent: 80,
      label: `Fast mode — synthesizing (0/${totalChunks})`,
      chunk: 0,
      totalChunks,
      fastMode: true,
    });
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    chunk.forEach((bookmark) => {
      ...
      const primaryFolder = pathParts[0] || 'Miscellaneous';
    ...
    if (onProgress) {
      ...
      onProgress({
        percent: progress,
        label: `Fast mode — staging chunk ${i + 1}/${totalChunks}`,
        chunk: i + 1,
        totalChunks,
        fastMode: true,
      });
  ...
  const folders = Array.from(foldersMap.entries()).map(([name, ids]) => ({
    name,
    ids: Array.from(ids),
