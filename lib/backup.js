...
export async function downloadLatest(flatListOverride = null) {
  try {
    const list = flatListOverride ?? (await readBookmarks()).flatList;
    await exportJson(list);
    await exportHtml(list);
    return { total: list.length };
  } catch (error) {
    console.error('[backup.js] Failed to download latest backups:', error);
    throw error;
  }
}
